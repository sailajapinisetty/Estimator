...
const buildGenerationSavings = (model, systemTokens, userTokens, outTokens, volume, allModels) => {
  ...
  steps.push({
    ...
    detail: `${model.name} · ${outTokens.toLocaleString()} response tokens`,
    assumption: null,
    costRaw: baseline,
  ...
  push(
    'Ask for the answer only — no intro, no explanation',
    `Response drops to ${outNow.toLocaleString()} tokens`,
    'Assumes about 15% of the response is filler the model adds on its own'
  );

  outNow = Math.max(50, Math.round(outTokens * 0.16));
  push(
    'Ask for a short structure and fill in the rest yourself',
    `Response drops to ${outNow.toLocaleString()} tokens`,
    'Assumes repeated boilerplate (imports, setup, page objects) is generated by your own template, not the model'
  );

  if (cheaper && cheaper.outputPricePerMillionTokens < model.outputPricePerMillionTokens) {
    modelNow = cheaper;
    push(
      `Send the easy requests to a cheaper model (${cheaper.name})`,
      `$${cheaper.pricePerMillionTokens} in / $${cheaper.outputPricePerMillionTokens} out`,
      'Assumes every request moves. If only some do, the real saving is smaller'
    );
  }

  multiplier = 0.5;
  push(
    'Send requests in bulk instead of one at a time',
    'Half price',
    'You get the results later, not instantly — like posting a stack of letters instead of paying for same-day delivery'
  );
...
// Small percentages need extra decimals so they don't collapse to "0.0"
...
router.post('/analyze', (req, res) => {
  try {
    ...
    const cacheSavingPercent = billedCost > 0 ? formatPercent((cacheSavingPerCall / billedCost) * 100) : '0';
    ...
    res.json({
      ...
      caching: isGeneration
        ? {
            ...
            savingPercent: billedCost ? formatPercent((cacheSavingPerCall / billedCost) * 100) : '0',
          }
        : null,
      volume: volume
        ? {
